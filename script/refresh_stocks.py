"""
refresh_stocks.py
Fetches live quotes from Yahoo Finance for COH.AX, SONVF, DEMANT.CO
plus AUD/USD, DKK/USD, EUR/USD FX rates and writes stocks.json.
Run by GitHub Actions daily; also used by the Perplexity cron.
"""
import urllib.request, json, datetime, os

YF_BASE = "https://query1.finance.yahoo.com"
HEADERS  = {"User-Agent": "Mozilla/5.0 (compatible; CI-Pulse/1.0)", "Accept": "application/json"}
OUT_PATH = os.path.join(os.path.dirname(__file__), "..", "stocks.json")
...
def fetch_fx(pair):
    try:
        ...
        v    = meta.get("regularMarketPrice") or meta.get("previousClose")
        print(f"  FX {pair} = {v}")
        return v
...
aud_usd = fetch_fx("AUDUSD=X") or 0.645
dkk_usd = fetch_fx("DKKUSD=X") or 0.145
eur_usd = fetch_fx("EURUSD=X") or 1.08

FX_TO_USD = {"AUD": aud_usd, "USD": 1.0, "DKK": dkk_usd, "EUR": eur_usd}

CI_STOCKS = [
    {"ticker": "COH.AX",    "name": "Cochlear Limited",  "currency": "AUD", "exchange": "ASX",
     "ciCompany": "Cochlear Ltd",            "role": "Direct"},
    {"ticker": "SONVF",     "name": "Sonova Holding AG", "currency": "USD", "exchange": "OTC",
     "ciCompany": "Sonova / Advanced Bionics","role": "Parent"},
    {"ticker": "DEMANT.CO", "name": "Demant A/S",        "currency": "DKK", "exchange": "Nasdaq Copenhagen",
     "ciCompany": "Demant / Oticon Medical", "role": "Parent"},
]

quotes  = []
history = {}

for stock in CI_STOCKS:
    ...
    print(f"  price={price_native} {native_currency}, hist_pts={len(hist)}")
